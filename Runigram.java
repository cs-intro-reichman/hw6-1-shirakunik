...
public class Runigram {
	...
	public static Color[][] flippedHorizontally(Color[][] image) {
		Color[][] imagehorizon = new Color[image.length][image[0].length];
		 

		for (int i =0; i<image.length; i++){
			for (int j =0; j<image[i].length; j++){
				 imagehorizon[i][image[0].length-1-j] = image[i][j];
			}		
		}
		return imagehorizon;
	}
	...
	public static Color[][] flippedVertically(Color[][] image){
		...
		for (int j =0; j<image[0].length; j++){
			for (int i =0; i<image.length; i++){
				 imagevertical[image.length-1-i][j] = image[i][j];
			}		
		}
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		double row = (double) image.length / height;
    	double col = (double) image[0].length / width;

		for (int i =0; i<height; i++){
			for (int j =0; j<width; j++){
			
				scaled[i][j] = image[(int)(i*row)][(int)(j*col)];
